<template>
    <div class="container">
        <scroll-view :scroll-y="true" :enable-back-to-top="true" style="height: 100%;" @scroll="scroll">
            <search />
            <div class="s-r-i">搜索关键字：<span style="color:red">{{keyword}}</span>，共<span style="color:red">{{total}}</span>条数据</div>
    ...
    watch: {
        scrollTop(curVal, oldVal) {
            if (this.scrollTop >= 450*this.page) {
                this.page ++
                var data = {'classid':this.classid,"keyword":this.keyword,"page":this.page,"limit":this.limit}
                this.loadMore(data)
            }
        }
    }
