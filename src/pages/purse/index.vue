  ...
  <div>
    ...
    <div class="section">
      <div class="section-1">
        ...
        <div class="s-2">
          <ul>
              <li v-for="(id,index) in item" :key="index" :class="{'bgStyle':index==currentIndex}" @click="change(id.id,index)" >{{ id.gname }}</li>
          </ul>
        </div>
      </div>
      <!-- <div class="section-2">
        ...
        <input type="number" placeholder="请输入其他金额" id="cash" @blur='cash'>
      </div>-->
    </div>
    <div class="footer">
      <p class="tip">温馨提示:</p>
...
export default {
  ...
  methods: {
    getCzm(userid){
       var that = this;
       wx.request({
      url: "https://qq.zhongyi9999.com/e/extend/api/index.php?m=user&c=amount&userid=userid",
      data: {
        userfen: "",
        userid: userid
      },
    ...
    cond() {
      ...
      //     for (var i = 0; i < itemArray.length; i++) {
      //       var obj = new Object();
      //       obj.cardfen = itemArray[i].cardfen;
      //       obj.buytime = itemArray[i].buytime;
      //       obj.type = itemArray[i].type;
      //       that.lists.push(obj);
      //       // that.$set(that.item1,itemArray,"obj")
      //     }
      //     console.log(obj.cardfen);
      //     console.log(that.item1);
      //   }
      // });

      wx.navigateTo({
        url: "../../pages/cond/main?userid=42"
      });
    },
    recd() {
      var that = this;
    ...
    cash(e) {
      //   var that = this;
      //   that.cash = parseInt(e.detail.value);
