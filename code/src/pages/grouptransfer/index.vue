     ...
     <div id="container">
			...
			<div class="Wrap-1">
				...
				<div @click="showSinglePicker" class="List-1"> 
						<p>所属行业选择</p>
						<div class="fan">{{pickerText2}}</div>
				</div>
			
				<div @click="showMulLinkageTwoPicker" class="List-2">
					<p>地区选择</p>
          	<div class="fan">{{pickerText}}</div>
				</div>
				...
				<div class="List-3">
					<picker @change="getPickerList3" :value="value3" :range="arrdata3">
						<p class="pickerone">标签</p>
				...
				<div class="Wecname">
					<p>微信群名称</p>
					<input type="text" id="WechatGroupName" placeholder="请输入名称" @blur="WechatGroupName"/>
				</div>
				<div class="Wecname1">
					<p>微信群主号</p>
					<input type="text" id="WechatGroupID" placeholder="请输入主号" @blur="WechatGroupID"/>
				</div>
				<div class="Wecname1">
					<p>转让群总人数</p>
					<input type="number" id="GroupTotal" placeholder="请输入总人数" @blur="GroupTotal"/>人
				</div>
				<div class="Wecname1">
					<p>转让群价格</p>
					<input type="number" id="GroupPrice" placeholder="请输入价格" @blur="GroupPrice"/>元/个
				</div>
				<div class="pic">
					<div class="Left">
						<p>微信头像</p>
					...
					<div class="Right">
						<p>群主二维码</p>
						<div class="D2" @click="Upfile2">
								<img :src="Twodimensional" v-if="Twodimensional" alt="" class="img">
						</div>
					</div>
				</div>
				<div class="introduction">
					<p>简介/介绍</p>
						<textarea v-model="content"  v-show="isInputContentFocus"  v-bind:focus="isFocus" id="introduction" cols="30" rows="10" class="introduction1" placeholder="请输入不少于15个文字的介绍，介绍越详细，图片越清晰，可为您带来更多浏览量。" @blur="introduction"></textarea>
						<div class="content" v-text="content" @click="bindContentFocus" v-show="isContentFocus">
        			<div id="placeholder" v-if="!content">请输入不少于15个文字的介绍，介绍越详细，图片越清晰，可为您带来更多浏览量。</div>
      			</div>
				</div>
			</div>
			...
			<div class="footer">
				...
				<div class="Right1" @click="release" :style="{background:(ismale ? '#56b7fb': '#56b7fb')}">立即发布</div>
			</div>
...
export default {
	...
	return {
			...
			Twodimensional:'../../static/img/icon1.png',
			tempFilePaths: '',
			value3:'',
			arrdata3:[],
			pickerText2:'',
	...
	mounted(data){
            ...
            wx.request({
						...
						success(data){
							...
							var field = hanyeList.select[0].field
							var type = hanyeList.select[2].field

							var classid = wx.getStorageSync('classid')
							var userid = wx.getStorageSync('userid')
							var gprice = hanyeList.info.gprice
							console.log(gprice)
							console.log(classid)
							...
							that.labelArray = hanyeList.select[2].option;
	...
	methods:{
	  Upfile(e){
		...
		getPickerList3(e){
			var that = this;
			that.value3 = e.mp.detail.value;
			var V3 = parseInt(that.value3)+1
			that.Vs = V3
			console.log(V3)
		},
		textShow(){
      ...
      this.$refs.mpvuePicker.show();
		...
		onChange(e) {
					...
					if(cityValue == undefined){
						that.IndustryValue = industryValue
						console.log(that.IndustryValue)
					}
		},
		onCancel(e) {
			var that = this;
        ...
        this.pickerText2 = e.label
			...
			WechatGroupName(e){
				 var that = this;
				 var WechatGroupName = e.mp.detail.value
				 that.Wn = WechatGroupName
				 console.log(WechatGroupName)
			},
			WechatGroupID(e){
				 var that = this;
				 var WechatGroupID = e.mp.detail.value
				 that.Wd = WechatGroupID
			},
			GroupTotal(e){
				 var that = this;
				 var GroupTotal = e.mp.detail.value
				 that.Gt = GroupTotal
			},
			GroupPrice(e){
				 var that = this;
				 var GroupPrice = e.mp.detail.value
				 that.Gp = GroupPrice
			},
			bindContentFocus(e) {
				this.isFocus = true; 
			...
			introduction(e){	 
				...
				var qrcode = that.Twodimensional

				if(Cty == "" || VS == "" || WN == ""  || titlepic == "" || ewm == "" || GT == "" || GP == "" || qrcode == ""){
							wx.showToast({
								title: '请核对',
								icon: 'loading',
								duration: 1000
					...
					if(Cty == "" || VS == "" || WN == ""  || titlepic == "" || ewm == "" || GT == "" || GP == "" || qrcode == ""){
						  ...
						  data: {
								 ...
								 method:'post',
									success(data){
												wx.navigateTo({	
													url: "../../pages/gotransferelease/main"
												});
											}	
							})
					}
