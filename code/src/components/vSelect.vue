    ...
    <section class="select_area" :style="areaStyle" v-if="isshow">
      ...
      <section class="options">
        <section class="option-area" v-for="(select,index) in data" :key="index">
          <section class="option" :data-value="option.value" :data-field="select.field" ref="dataValue" v-for="(option,key) in select.option" 
          :key="key" v-show="showOptions[index].show" @click="getChange($event,index,key)">
...
export default {
  ...
  methods: {
    ...
    getChange(e, index, key) {
      ...
      this.result[e.currentTarget.dataset.field] =
